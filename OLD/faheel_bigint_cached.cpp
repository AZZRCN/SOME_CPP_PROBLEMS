...
class BigInt {
    ...
    char sign;
        ...
        bool operator>=(const std::string&) const;
        bool operator==(const std::string&) const;
        bool operator!=(const std::string&) const;        friend std::istream& operator>>(std::istream&, BigInt&);
        friend std::ostream& operator<<(std::ostream&, const BigInt&);        std::string to_string() const;
        int to_int() const;
        long to_long() const;
        long long to_long_long() const;        friend BigInt big_random(size_t);
};
#endif
...
BigInt big_random(size_t num_digits = 0) {
    ...
    BigInt big_rand;
    big_rand.value = "";    big_rand.value += std::to_string(1 + rand_generator() % 9);
...
std::string BigInt::to_string() const {return this->sign == '-' ? "-" + this->value : this->value;}
...
BigInt sqrt(const BigInt& num) {
    if (num < 0)
        throw std::invalid_argument("Cannot compute square root of a negative integer");    if (num == 0)
        return 0;
    else if (num < 4)
        return 1;
    else if (num < 9)
        ...
        return 3;
    BigInt sqrt_prev = -1;    BigInt sqrt_current = big_pow10(num.to_string().size() / 2 - 1);
...
BigInt gcd(const BigInt &num1, const BigInt &num2){
    ...
    BigInt abs_num2 = abs(num2);    if (abs_num2 == 0)
        return abs_num1;
    if (abs_num1 == 0)
        return abs_num2;
...
BigInt lcm(const BigInt& num1, const BigInt& num2) {if (num1 == 0 or num2 == 0){return 0;}return abs(num1 * num2) / gcd(num1, num2);}
...
BigInt BigInt::operator+(const BigInt& num) const {    if (this->sign == '+' and num.sign == '-') {
        BigInt rhs = num;
        rhs.sign = '+';
        return *this - rhs;
    }
    ...
    BigInt result;
    result.value = "";
    short carry = 0, sum;    for (long i = larger.size() - 1; i >= 0; i--) {
        sum = larger[i] - '0' + smaller[i] - '0' + carry;
        result.value = std::to_string(sum % 10) + result.value;
        carry = sum / (short) 10;
    }
    if (carry)
        result.value = std::to_string(carry) + result.value;    if (this->sign == '-' and result.value != "0")
        result.sign = '-';
    return result;
}
BigInt BigInt::operator-(const BigInt& num) const {    if (this->sign == '+' and num.sign == '-') {
    ...
    BigInt result;    std::string larger, smaller;
    if (abs(*this) > abs(num)) {
        larger = this->value;
        smaller = num.value;
        if (this->sign == '-')
    ...
    result.value = "";
    short difference;
    long i, j;    for (i = larger.size() - 1; i >= 0; i--) {
    ...
    strip_leading_zeroes(result.value);    if (result.value == "0")
        result.sign = '+';
    return result;
}
BigInt BigInt::operator*(const BigInt& num) const {
    ...
    else {        std::string larger, smaller;
        std::tie(larger, smaller) = get_larger_and_smaller(this->value, num.value);
...
BigInt BigInt::operator%(const BigInt& num) const {
    ...
    strip_leading_zeroes(remainder.value);    remainder.sign = this->sign;
...
BigInt& BigInt::operator/=(const BigInt& num) {*this = *this / num;return *this;}
BigInt& BigInt::operator%=(const BigInt& num) {*this = *this % num;return *this;}
BigInt& BigInt::operator+=(const long long& num) {*this = *this + BigInt(num);return *this;}
BigInt& BigInt::operator-=(const long long& num) {*this = *this - BigInt(num);return *this;}
BigInt& BigInt::operator*=(const long long& num) {*this = *this * BigInt(num);return *this;}
BigInt& BigInt::operator/=(const long long& num) {*this = *this / BigInt(num);return *this;}
BigInt& BigInt::operator%=(const long long& num) {*this = *this % BigInt(num);return *this;}
BigInt& BigInt::operator+=(const std::string& num) {*this = *this + BigInt(num);return *this;}
BigInt& BigInt::operator-=(const std::string& num) {*this = *this - BigInt(num);return *this;}
BigInt& BigInt::operator*=(const std::string& num) {*this = *this * BigInt(num);return *this;}
BigInt& BigInt::operator/=(const std::string& num) {*this = *this / BigInt(num);return *this;}
BigInt& BigInt::operator%=(const std::string& num) {*this = *this % BigInt(num);return *this;}
